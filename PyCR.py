...
matplotlib.use('agg')
def main(isexternal,howMuchSplit,isMicro,tupaType,isMotabo,MotaboFileName,DataFileName,ClassFileName,sampleNameFile,variableNameFile,scale_type,iteration,survivalRate,V_rankingAlgorithm, nComponent):
    ITERATION = iteration
    ...
    OUTPUT_PATH =  file_pkg.create_folder()
    if isMotabo:
    ...
    file_pkg.gen_file_for_one_line(valid_idx,OUTPUT_PATH+'/selcted_index.csv')
    if isexternal:
    ...
    if isexternal:
        ...
        plt.rcParams.update({'font.size': 10})
        plt.legend()
        plt.savefig(OUTPUT_PATH + '/pcls_training.png')
        if isexternal:
        ...
        plt.rcParams.update({'font.size': 10})
        plt.legend()
        plt.savefig(OUTPUT_PATH + '/pcls_external.png')
        plt.figure().clear()
    ...
    gen_pcls(scale_training_sample, classNum, class_index_list, CLASS_COLOR, CLASS_LABEL,
             OUTPUT_PATH + '/PCLSTrainNoFS.png', 'PLS-DA Training, No Feature Selection ', class_trans_dict, classList)
    ...
    if isexternal:
        ...
        gen_pcls(scaled_external, classNum, external_class_index_list, CLASS_COLOR, CLASS_LABEL,
                 OUTPUT_PATH + '/PCLSValiNoFS.png', 'PLS-DA Validation, No Feature Selection ', class_trans_dict,
                 external_class)
        # generate predict ROC
        if classNum == 2:
    ...
    gen_pcls(scale_training_sample[:, valid_idx], classNum, class_index_list, CLASS_COLOR, CLASS_LABEL,
             OUTPUT_PATH + '/PCLSTrainWithFS.png', 'PLS-DA Training, With Feature Selection ', class_trans_dict,
             classList)
    # generate predict ROC
    if classNum == 2:
    ...
    if isexternal:
        ...
        gen_pcls(scaled_external[:, valid_idx], classNum, external_class_index_list, CLASS_COLOR, CLASS_LABEL,
                 OUTPUT_PATH + '/PCLSValiWithFS.png', 'PLS-DA Validation, With Feature Selection ', class_trans_dict,
                 external_class)
        # generate predict ROC
        if classNum == 2:
    ...
    training_sample_pcla_no_FS = np.array(scale_training_sample)
    ...
    if isexternal:
        ...
        plt.rcParams.update({'font.size': 10})
        plt.legend()
        plt.savefig(OUTPUT_PATH + '/pcls_no_FS.png')
        plt.figure().clear()
...
def runPyCR(isexternal,rateSplit,isMicro,tupaType,isMotabo,motaboFileName,dataFileName,classFileName,sampleNameFileName,variableNameFileName,scaleType,howManyIteration,survivalrate,V_rankingAlgorithm,nComponent):
    if isexternal == 'true':
        isexternal = True
    else:
        isexternal = False
